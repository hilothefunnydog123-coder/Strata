 ...
 *
 * Callable from the operator console as well as from the command line, because
 * a deployment on a host with no shell would otherwise have no way to get
 * anything into it, and that host is the common case now.
 *
 * Everything written here is synthetic and tagged as such: the hospital, the
 * patients, the payer and the two appeal decisions are all invented, and every
 * denial carries is_synthetic, so the same upload rule that protects real
 * records applies to this data too.
 *
 * What is not faked is the verification. Every quote is produced by slicing it
 * out of the passage it cites, so it cannot drift through a typo, and is then
 * put through the production verifyQuote against the real source text. If any
 * quote fails, nothing is written at all. The letter view's click-to-source,
 * the highlight offsets and the reviewer checklist therefore exercise the same
 * code paths on this data as they would on a real case. A demonstration that
 * stubbed that check would be demonstrating the one thing this product cannot
 * afford to fake.
 *
 * The two regulation passages are genuine federal text. The two decisions are
 * written for the demonstration and cited DEMO-DAB-0001 and DEMO-DAB-0002 so
 * they cannot be mistaken for precedent.
 */
...
async function seedUsers(STAMP: number, credentials: SeededCredential[]) {
  const mk = async (
    role: string,
    email: string,
    name: string,
...
export async function seedDemo(options: { reset?: boolean } = {}): Promise<SeedResult> {
  ...
  const users = await seedUsers(STAMP, credentials);
  ...
  return {
    organisation: 'Northgate Regional Medical Center',
    contingencyRateBps: RATE_BPS,
    denials: 4,
    assertions: prepared.length,
    invoiceNumber: invoiceNumber(ORG_SLUG, period.start, 1),
    feeCents: calc.feeCents,
    recoveredCents: calc.totalRecoveredCents,
    credentials,
  };
}
