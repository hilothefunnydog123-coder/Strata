...
| Hardening | Auth + transport | ✅ proven | **Failure throttling** on login and enrollment: per-IP denies (12/15min), per-account only *slows* (progressive delay, 60/15min backstop). The first design hard-locked the account and a brute-force test then refused the CORRECT password from a clean IP — a stranger who knew the address could lock the owner out, so it was rebuilt. Verified: 12 wrong guesses → 401s that slow 0.05s→1.3s then 429, while the owner still signs in (200) from another address. **Security headers** from `next.config.mjs` so they travel with the app: CSP (`frame-ancestors 'none'`, `object-src 'none'`, `base-uri 'self'`), HSTS, nosniff, DENY, Referrer-Policy, Permissions-Policy, no `x-powered-by`. **Fonts self-hosted** from npm — a real browser found the CSP blocking Google Fonts, and self-hosting closed it without opening the policy: 3/3 families load, **zero off-origin requests**. **Demo seed no longer ships otplib's published TOTP secret** (was signable-into by anyone who read the repo) and gets a random unusable password unless `OWNER_PASSWORD` is set — old public credential now 401. |
...
| M7 | Desktop shell + device auth | ◑ built + **runnable from any browser** | `/terminal` serves the same Vite bundle the Tauri shell loads, behind the console session — every asset goes through the session check, not just the entry page, and path traversal is rejected. The renderer re-flows below 900px (rail → scrollable tabs, split panes stack, wide tables scroll in their own box): all six modules verified at 390×844 with **0px horizontal overflow**, tables confirmed scrollable, and the ≥900px layout byte-for-byte unchanged. Device pairing is skipped when the console is the host, since the request already carried a verified session. |
| M7 (native) | Tauri packaging | ◑ built, not packaged | Tauri v2 project + React frontend build; the frontend **runs and is screenshotted** (device-auth screen, Criteria Rail, Coverage Map, Evidence Blueprint, Change Watch — `docs/screenshots/06–10`); device-flow API + approval work end-to-end on the web side. Not compiled to a notarized mac/Windows binary here (no display / signing toolchain). |
