...
| M6 | Auth + dashboard | ✅ proven | admin CLI provisions accounts — `pnpm db:seed` for the demo fixture, `pnpm founder --email …` for a real credential (generated password + private TOTP secret, shown once, scrypt-hashed at rest, `--rotate` revokes live sessions and paired desktops). Password + TOTP login reaches the dashboard, verified headless end-to-end: 200 + httpOnly cookie, `/dashboard` renders the founder's org, anonymous requests 307 to `/login`, wrong password and wrong TOTP both 401, and a rotated password is rejected while the new one is accepted. Download/license/seats/invoices + read-only coverage summary; **no signup route exists** |
